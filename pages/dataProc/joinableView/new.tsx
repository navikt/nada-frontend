...
import { Dataproduct, SearchType, useAccessiblePseudoDatasetsQuery, useSearchContentWithOptionsQuery } from '../../../lib/schema/graphql'
  ...
  return (
    <div className="mt-8 md:w-[46rem]">
      ...
      >
        ...
        {
          srcDatasets.map((it, index) => <div key={index} className='flex flex-row items-center gap-2'>
            <Select
              className="w-full"
              label="Velg et view"
              value={it}
              ...
              {pseudoDatasets && [
                ...new Set(
                  pseudoDatasets?.filter(it => !srcDatasets.some((sdsid, sdsindex) => sdsid == it.datasetID && index != sdsindex))
                    .map((it: any) => <option value={it.datasetID} key={it.datasetID}>{it.name}</option>)
                ),
              ]}
            </Select>
        ...
        {pseudoDatasets && pseudoDatasets.length > srcDatasets.length && <Link href='#' onClick={() => setSrcDatasets([...srcDatasets, ""])}>
