import { Button, ErrorSummary, Heading, Radio, RadioGroup, TextField } from '@navikt/ds-react'
import { Controller, useForm } from 'react-hook-form'
import { yupResolver } from '@hookform/resolvers/yup/dist/yup'
import { useState } from 'react'
import * as yup from 'yup'
import { DatasetQuery, useUpdateDatasetMutation, UpdateDataset } from '../../../lib/schema/graphql'
...
const EditDataset = ({ dataset, setEdit }: EditDatasetFormProps) => {
    ...
    return (
        <div className="block pt-8 pr-8">
            <Heading level="1" size="medium" spacing>Endre datasett</Heading>
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-10 h-[90%]">
                <TextField
                    {...register("name")}
                ...
                />
                <TextField label="Dataset" disabled value={bigquery.projectID+"."+bigquery.dataset+"."+bigquery.table}/>
                <KeywordsInput
                    onAdd={onAddKeyword}
                ...
                <div className="flex flex-row gap-4 grow items-end">
                    <Button type='button' variant='secondary' onClick={() => { setEdit(false) }}>Avbryt</Button>
