...
const DatasetAccess = ({ dataset, access, error, loading }: AccessProps) => {
    ...
    return (<>
        {access.accessRequestsForDataset.length > 0 && <Section>
            <Heading spacing level="3" size="small">Tilganger</Heading>
            <ul>
                {access.accessRequestsForDataset.map(a => <li key={a.id}>
                    {a.subject}
                </li>)}
            </ul>
        </Section>}
        {dataset.access.length > 0 &&
            <Section>
                <Heading spacing level="3" size="small">Aktive tilganger</Heading>
                <ul>
                    {dataset.access.map(a => <li key={a.id}>
