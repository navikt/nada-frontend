      ...
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>
            ...
            <TableCell key="canRequest" align="center">
              Kan gi seg selv tilgang
            </TableCell>
            <TableCell key="hasAccess" align="center">
              Har tilgang
            </TableCell>
            <TableCell key="removeAccess" align="center">
              Fjern tilgang
            </TableCell>
            <TableCell key="viewRequest" align="center">
              Se søknad
            </TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {accesses.map((a, i) => (
            <TableRow key={i}>
              ...
              <TableCell key={'can-request' + i} align="center">
                {a.canRequest ? (
                  <Success style={{ color: navGronn }} />
                ) : (
                  <Error style={{ color: navRod }} />
                )}
              </TableCell>
              <TableCell key={'has-access' + i} align="center">
                {a.access ? (
                  <>
                    {a.access.expires ? humanizeDate(a.access.expires) : 'evig'}
                  </>
                ) : (
                  <Error style={{ color: navRod }} />
                )}
              </TableCell>
              <TableCell key={'remove-access' + i} align="center">
                <Delete
                  style={{ cursor: 'pointer', color: navRod }}
                  onClick={() => removeAccess(id, a)}
                />
              </TableCell>
              <TableCell key={'view-request' + i} align="center">
                {a.access?.accessRequestID ? (
                  <Search
                    style={{ cursor: 'pointer', color: navBla }}
                    onClick={() =>
                      router.push(`/request/${a.access?.accessRequestID}/view`)
                    }
                  />
                ) : (
                  'Ingen søknad'
