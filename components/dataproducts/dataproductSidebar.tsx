    ...
    <div className="flex flex-col items-stretch justify-between pt-8 w-64">
      <div className="flex w-64 flex-col gap-2">
        {menuItems.map(({ title, slug }, idx) =>
          currentPage == idx ? (
            <p
              className="border-l-[6px] border-l-link py-1 px-2 font-semibold"
              key={idx}
            >
              {title}
            </p>
            ...
            <a
              className="border-l-[6px] border-l-transparent font-semibold no-underline hover:underline hover:cursor-pointer py-1 px-2"
              href="#"
              key={idx}
